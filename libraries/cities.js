...
function print_city(city_id, selectedcity) {
  var city_index = state_arr.indexOf(selectedcity) + 1;
  console.log('index - ' + city_index);
...
function getCitiesforState(state) {
  var city_index = state_arr.indexOf(state) + 1;
  var cityValueArray = [];
  var city_arr = s_a[city_index].split('|');
  for (var i = 0; i < city_arr.length; i++) {
    cityValueArray.push({ value: city_arr[i], label: city_arr[i] });
  }
  return cityValueArray;
}

export { print_state, print_city, getCitiesforState };
