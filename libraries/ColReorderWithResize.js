...
(function ($, window, document) {
  ...
  ColReorder = function (oDTSettings, oOpts) {
    ...
    oDTSettings.oApi._fnCallbackReg(
      oDTSettings,
      'aoDestroyCallback',
      jQuery.proxy(this._fnDestroy, this),
      'ColReorder'
    );
  ...
  ColReorder.prototype = {
    ...
    _fnStateSave: function (oState) {
      ...
      for (i = 0; i < oState.aaSorting.length; i++) {
        oState.aaSorting[i][0] =
          oSettings.aoColumns[oState.aaSorting[i][0]]._ColReorder_iOrigCol;
      }
      ...
      if (this.s.allowResize) {
        $(nTh).bind('mousemove.ColReorder', function (e) {
          if (that.dom.drag === null && that.dom.resize === null) {
            ...
            } else $(nThTarget).css({ cursor: 'pointer' });
          }
        });
      }
    ...
    _fnMouseDown: function (e, nTh, i) {
      ...
      if ($(nTh).css('cursor') == 'col-resize') {
        ...
        /* Store information about the mouse position */
        var nThTarget =
          e.target.nodeName == 'TH' ? e.target : $(e.target).parents('TH')[0];
        var offset = $(nThTarget).offset();
        this.s.mouse.startX = e.pageX;
        ...
        this.s.mouse.fromIndex =
          this.s.dt.oInstance.oApi._fnVisibleToColumnIndex(
            this.s.dt,
    ...
    _fnMouseMove: function (e, colResized) {
      ...
      if (
        this.table_size < 0 &&
        scrollXEnabled &&
        $('div.dataTables_scrollHead', this.s.dt.nTableWrapper) != undefined
      ) {
        if ($('div.dataTables_scrollHead', this.s.dt.nTableWrapper).length > 0)
          this.table_size = $(
            $('div.dataTables_scrollHead', this.s.dt.nTableWrapper)[0]
      ...
      if (this.dom.resize) {
        ...
        if (
          ...
          if (
            $('div.dataTables_scrollHead', this.s.dt.nTableWrapper).length > 0
          )
            $(
              $('div.dataTables_scrollHead', this.s.dt.nTableWrapper)[0]
                .childNodes[0].childNodes[0]
            ).width(this.table_size + moveLength);
        }
        ...
        if ($('div.dataTables_scrollBody') != null) {
          ...
          if ($('div.dataTables_scrollBody').length > 0) {
            ...
            for (
              ...
              currentColumnIndex < this.s.dt.aoColumns.length - 1 &&
              currentColumnIndex != colResized;
              currentColumnIndex++
            ) {
            ...
            if (moveLength != 0 && !scrollXEnabled) {
              $(
                $(scrollingTableHead)[0].childNodes[visibleColumnIndex + 1]
              ).width(this.s.mouse.nextStartWidth - moveLength);
            }
            $($(scrollingTableHead)[0].childNodes[visibleColumnIndex]).width(
              this.s.mouse.startWidth + moveLength
            );

            //Resize the table too
            if (scrollXEnabled)
              $($(tableScroller)[0].childNodes[0]).width(
                this.table_size + moveLength
              );
          }
        }
        ////////////////////////
        ...
        if (!bSet) {
          this.dom.pointer.style.left =
            this.s.aoTargets[this.s.aoTargets.length - 1].x + 'px';
    ...
    _fnMouseUp: function (e, colResized) {
      ...
      //Martin Marchetta
        ...
        this.s.dt.aoColumns[colResized].sWidth =
          $(this.s.mouse.resizeElem).innerWidth() + 'px';
        ...
        if (!scrollXEnabled) {
          ...
          for (
            ...
            if (this.s.dt.aoColumns[nextVisibleColumnIndex].bVisible) break;
          }
          ...
          for (
            ...
            if (this.s.dt.aoColumns[previousVisibleColumnIndex].bVisible) break;
          }

          if (this.s.dt.aoColumns.length > nextVisibleColumnIndex)
            this.s.dt.aoColumns[nextVisibleColumnIndex].sWidth =
              $(this.s.mouse.resizeElem).next().innerWidth() + 'px';
          else {
            ...
            for (i = previousVisibleColumnIndex; i > 0; i--) {
              if (this.s.dt.aoColumns[i].bVisible) {
                ...
                this.s.dt.aoColumns[i].sWidth =
                  $(currentColumn).innerWidth() + 'px';
              }
            }
          }
        }
    ...
    _fnCreateDragNode: function () {
      ...
      while (this.dom.drag.getElementsByTagName('caption').length > 0) {
        this.dom.drag.removeChild(
          this.dom.drag.getElementsByTagName('caption')[0]
        );
      }
      while (this.dom.drag.getElementsByTagName('tbody').length > 0) {
        this.dom.drag.removeChild(
          this.dom.drag.getElementsByTagName('tbody')[0]
        );
      }
      while (this.dom.drag.getElementsByTagName('tfoot').length > 0) {
        this.dom.drag.removeChild(
          this.dom.drag.getElementsByTagName('tfoot')[0]
        );
      }
      ...
      $('thead th:eq(0)', this.dom.drag).each(function (i) {
        this.style.width =
          $(
            'th:eq(' + that.s.mouse.targetIndex + ')',
            that.s.dt.nTHead
          ).width() + 'px';
      });
      ...
      if (this.s.dt.oScroll.sX === '' && this.s.dt.oScroll.sY === '') {
        ...
        this.dom.pointer.style.top =
          $('div.dataTables_scroll', this.s.dt.nTableWrapper).offset().top +
          'px';
        this.dom.pointer.style.height =
          $('div.dataTables_scroll', this.s.dt.nTableWrapper).height() + 'px';
      }
    ...
    $.fn.dataTableExt.aoFeatures.push({
      fnInit: function (oDTSettings) {
        ...
        if (typeof oTable._oPluginColReorder == 'undefined') {
          var opts =
            typeof oDTSettings.oInit.oColReorder != 'undefined'
